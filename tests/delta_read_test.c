...
static void run_test(size_t input_size, bool be_bad)
 {
    Memory input;
    size_t i;
    ...
    /* May need to try several times for this to complete.
     * See https://github.com/librsync/librsync/issues/33. */
    do {
        r1 = rs_job_iter(job, &buf);
    } while (r1 == RS_BLOCKED);
